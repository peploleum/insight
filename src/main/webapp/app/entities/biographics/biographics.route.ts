...
export class BiographicsResolve implements Resolve<IBiographics> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Biographics> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<Biographics>) => response.ok),
                map((biographics: HttpResponse<Biographics>) => biographics.body)
            );
        }
